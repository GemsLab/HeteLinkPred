...
from utils import Logger, to_bidirected_with_reverse_mapping, load_usair_dataset, load_esci_dataset, \
    remove_collab_dissimilar_edges, load_grid_dataset, load_dense_grid_dataset, debug_attach, index_to_mask, \
        load_custom_dgl_dataset
...
from models import GPRGNN_DOT, SAGE, SAGE_DOT, GPRGNN_DistMultS, SAGE_DistMultS, GCN, GATv2, GIN, \
    GCN_DOT, GCN_DistMultS, MLPDecoder, DistMultSDecoder, GPRGNN
import os
import numpy as np
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for epoch in epoch_progress:
            ...
            for it, (input_nodes, pair_graph, neg_pair_graph, blocks) in it_progress:
                ...
                # if args.model_name == "GPRGNN":
                #     bg = dgl.to_bidirected(g)
                #     u, v = bg.edges()
                #     num_nodes = bg.num_nodes()
                #     # print(u, v)
                #     edge_index = EdgeIndex(
                #         torch.stack((u, v)), sparse_size=(num_nodes, num_nodes), sort_order='row', is_undirected=True).to(device)
                #     edge_index.validate()
                #     # print(edge_index)
                #     assert edge_index.max() < num_nodes
                #     pos_score, neg_score = model(
                #         pair_graph, neg_pair_graph, blocks, x, edge_index)
                # else:
            ...
            if (epoch + 1) % args.log_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    for split in ['valid', 'test']:
                        ...
                        elif args.dataset in ["esci" , "grid", "grid-dense", "amazon-computer", "attributed-ppi", "attributed-facebook"]:
                ...
                if args.dataset in ("ogbl-citation2", "esci", "grid", "grid-dense", "amazon-computer", "attributed-ppi", "attributed-facebook"):
...
if __name__ == '__main__':
    ...
    if args.dataset == 'USAir':
        ...
        g, reverse_eids, seed_edges, edge_split = load_dense_grid_dataset(device, 
            dataset_root="./dataset/dense_datasets_new_jiong", graph_idx=args.grid_idx)
    elif args.dataset == 'esci':
        g, reverse_eids, seed_edges, edge_split = load_esci_dataset(device, dataset_root="./dataset/esci")
    elif args.dataset == 'amazon-computer':
        g, reverse_eids, seed_edges, edge_split = load_custom_dgl_dataset(
            device, "dataset/AmazonComputer_lp", "AmazonComputer")
    elif args.dataset == "attributed-ppi":
        g, reverse_eids, seed_edges, edge_split = load_custom_dgl_dataset(
            device, "dataset/attributed_PPI", "AttributedGraphPPI")
    elif args.dataset == "attributed-facebook":
        g, reverse_eids, seed_edges, edge_split = load_custom_dgl_dataset(
            device, "dataset/attributed_Facebook", "AttributedGraphFacebook")
    ...
    for run in range(args.runs):
        ...
        if args.model_name == "SAGE":
            ...
            g = dgl.add_self_loop(g)
            print("Add self loop")
            model = GPRGNN(in_size, args.hidden_dim, alpha=args.alpha).to(device)
        elif args.model_name == "GPRGNN_DOT":
            g = dgl.remove_self_loop(g)
            g = dgl.add_self_loop(g)
            print("Add self loop")
            model = GPRGNN_DOT(in_size, args.hidden_dim, alpha=args.alpha).to(device)
        elif args.model_name == "GPRGNN_DistMultS":
            g = dgl.remove_self_loop(g)
            g = dgl.add_self_loop(g)
            print("Add self loop")
            model = GPRGNN_DistMultS(in_size, args.hidden_dim, alpha=args.alpha).to(device)
        else:
