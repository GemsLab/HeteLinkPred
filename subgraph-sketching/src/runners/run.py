...
from src.data import get_amazon_computer_data, get_amazon_computer_loaders_BUDDY, get_custom_data, get_custom_loaders_BUDDY, get_data, get_esci_data, get_esci_loaders_BUDDY, get_loaders, get_grid_data, get_grid_loaders_BUDDY
...
def run(args):
    ...
    for rep in range(args.reps):
        ...
        elif args.dataset_name=="amazon-computer":
            dataset, splits, directed, eval_metric = get_amazon_computer_data(args, device, "./dataset/AmazonComputer_lp/")
            train_loader, train_eval_loader, val_loader, test_loader = get_amazon_computer_loaders_BUDDY(args, dataset, device, splits)
        ...
        elif args.dataset_name == "attributed-ppi":
            dataset, splits, directed, eval_metric = get_custom_data(args, device, "./dataset/attributed_PPI/", "AttributedGraphPPI")
            train_loader, train_eval_loader, val_loader, test_loader = get_custom_loaders_BUDDY(args, dataset, args.dataset_name, splits)
        elif args.dataset_name  == "attributed-facebook":
            dataset, splits, directed, eval_metric = get_custom_data(args, device, "./dataset/attributed_Facebook/", "AttributedGraphFacebook")
            train_loader, train_eval_loader, val_loader, test_loader = get_custom_loaders_BUDDY(args, dataset, args.dataset_name, splits)
        else:
...
def select_model(args, dataset, emb, device):
    if args.model == 'SEALDGCNN':
        ...
        elif args.dataset_name in {"amazon-computer", "attributed-ppi", "attributed-facebook"}:
            model = BUDDY(args, dataset["feat"].size(1), node_embedding=emb).to(device)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='Cora',
                        choices=['Cora', 'Citeseer', 'Pubmed', 'ogbl-ppa', 'ogbl-collab', 'ogbl-ddi',
                                 'ogbl-citation2', 'grid', "esci", "amazon-computer", "attributed-ppi", "attributed-facebook"])
